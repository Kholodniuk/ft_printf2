...
/*   Updated: 2017/03/16 12:39:25 by skholodn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int     parse_color(t_e *e, char *fmt, va_list ar)
{
    if (ft_strstr_stop(fmt, BLACK_COLOR))
        return ((int)write(e->fd, BLACK, ft_strlen(BLACK)));
    else if (ft_strstr_stop(fmt, RED_COLOR))
        return ((int)write(e->fd, RED, ft_strlen(RED)));
    else if (ft_strstr_stop(fmt, GREEN_COLOR))
        return ((int)write(e->fd, GREEN, ft_strlen(GREEN)));
    else if (ft_strstr_stop(fmt, BROWN_COLOR))
        return ((int)write(e->fd, BROWN, ft_strlen(BROWN)));
    else if (ft_strstr_stop(fmt, BLUE_COLOR))
        return ((int)write(e->fd, BLUE, ft_strlen(BLUE)));
